...
mineBlock miner parent txs =
  let coinbase = coinbaseTx miner
   in let txroot = treeHash $ buildTree (coinbase : txs)
       in let getBlock nonce = BlockHeader {parent = parent, coinbase = coinbase, txroot = txroot, nonce = nonce}
           in let tryGetBlock nonce =
                    let block = getBlock nonce
                     in if validNonce block then block else tryGetBlock (nonce + 1)
               in let header = tryGetBlock 0
                   in Block {blockHdr = header, blockTxs = txs}
...
verifyChain = verifyChainHelper . reverse

verifyChainHelper :: [Block] -> Maybe Hash
verifyChainHelper [] = Just 0
verifyChainHelper [b] = verifyBlock b (parent $ blockHdr b)
verifyChainHelper (parentBlock : childBlock : t) =
  if isJust (verifyChainHelper [parentBlock]) && isJust (verifyBlock childBlock (hash parentBlock))
    then verifyChainHelper (childBlock : t)
    else Nothing
